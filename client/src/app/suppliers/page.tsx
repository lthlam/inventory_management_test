...
import { SupplierOption } from '../../types/inventory';
...
export default function SuppliersPage() {
  ...
  const columns = useMemo<ColumnDef<SupplierOption>[]>(
    () => [
      {
        accessorKey: 'name',
        header: 'Nhà cung cấp',
        size: 320,
        meta: { filterPlaceholder: 'Tìm nhà cung cấp...' },
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div>
        <div>
          <h1 className="text-3xl font-bold text-white">Danh sách nhà cung cấp</h1>
          <p className="text-slate-400 mt-1">Các nhà cung cấp hiện có trong hệ thống</p>
        </div>
      </div>
      ...
      <div className="table-frame bg-slate-800 border border-slate-700 overflow-hidden">
          ...
          <table className="table-grid w-full table-fixed border-collapse text-left">
            ...
            <tbody>
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={totalColumns} className="text-center py-12 text-slate-500">
                    {hasActiveFilters ? 'Không tìm thấy nhà cung cấp phù hợp' : 'Chưa có nhà cung cấp nào.'}
                  </td>
                </tr>
              ) : (
        ...
        {!isLoading && !error && (
          <div className="p-4 border-t border-white/10 bg-slate-900/30 text-sm text-slate-400">
            Tổng cộng <span className="font-medium text-slate-200">{filteredCount}</span> kết quả {hasActiveFilters && `(lọc từ ${totalSuppliersCount})`}
          </div>
        )}
      </div>
    </div>
