...
import { DelivererOption } from '../../types/inventory';
...
export default function DeliverersPage() {
  const { data: deliverers = [], error, isLoading } = useQuery({
    queryKey: ['deliverers'],
  ...
  const columns = useMemo<ColumnDef<DelivererOption>[]>(
    () => [
      {
        accessorKey: 'name',
        header: 'Người giao hàng',
        size: 320,
        meta: { filterPlaceholder: 'Tìm người giao hàng...' },
  ...
  const totalDeliverersCount = deliverers.length;
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div>
        <div>
          <h1 className="text-3xl font-bold text-white">Danh sách người giao hàng</h1>
          <p className="text-slate-400 mt-1">Các người giao hiện có trong hệ thống</p>
        </div>
      </div>
      ...
      <div className="table-frame bg-slate-800 border border-slate-700 overflow-hidden">
              ...
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={totalColumns} className="text-center py-12 text-red-400">
                    Không tải được danh sách người giao hàng.
                  </td>
                </tr>
              ) : table.getRowModel().rows.length === 0 ? (
                <tr>
                  <td colSpan={totalColumns} className="text-center py-12 text-slate-500">
                    {hasActiveFilters ? 'Không tìm thấy người giao hàng phù hợp' : 'Chưa có người giao hàng nào.'}
                  </td>
                </tr>
              ) : (
        ...
        {!isLoading && !error && (
          <div className="p-4 border-t border-white/10 bg-slate-900/30 text-sm text-slate-400">
            Tổng cộng <span className="font-medium text-slate-200">{filteredCount}</span> kết quả {hasActiveFilters && `(lọc từ ${totalDeliverersCount})`}
          </div>
        )}
      </div>
    </div>
